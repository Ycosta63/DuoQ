    ...
    <section className="flex-1 relative flex flex-col items-center justify-center p-6 overflow-hidden">
      <AnimatePresence>
        <motion.div 
          ...
          className={`relative w-full max-w-2xl max-h-[75vh] min-h-[75vh] bg-[#111] rounded-3xl overflow-hidden border border-[#333] shadow-2xl flex flex-col`}
        >
          {/* Main Visual Section */}
          <div className={`relative ${showDetails ? 'h-48' : 'flex-1'} transition-all duration-500 bg-[#222] shrink-0`}>
            ...
            <div className={`absolute left-8 z-20 transition-all duration-500 ${showDetails ? 'bottom-4' : 'bottom-8'}`}>
              <h2 className={`font-black italic tracking-tighter uppercase mb-1 leading-none text-white ${showDetails ? 'text-3xl' : 'text-5xl'}`}>
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {currentProfile.gender && (
                    <>
                      ...
                      <span className="text-[#AAA] text-sm uppercase font-semibold tracking-wider">{currentProfile.gender}</span>
                    </>
                  )}
                </div>
              )}
            ...
            <button 
              ...
              className="absolute bottom-4 right-8 z-30 w-10 h-10 rounded-full bg-white/10 backdrop-blur-md border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-colors"
            >
          ...
          {showDetails && (
            <motion.div 
              ...
              className="flex-1 overflow-y-auto custom-scrollbar bg-[#0A0A0A] p-8 -mt-2 relative z-20"
            >
            ...
            <div className="h-24 bg-[#0A0A0A] border-t border-[#2A2A2A] px-8 py-4 flex items-center justify-between shrink-0">
              <div className="flex gap-3 overflow-x-auto custom-scrollbar overflow-y-hidden pb-1">
                {currentProfile.games.split(',').map((game, i) => game.trim() ? (
                  <div key={i} className="px-4 py-2 bg-[#1A1A1A] border border-[#333] rounded-lg text-[10px] uppercase font-bold text-[#AAA] whitespace-nowrap">
                    {game.trim()}
                  </div>
                ) : null)}
              </div>
      ...
      <div className="absolute bottom-6 flex gap-6 z-30 pointer-events-auto">
        <button 
